...
from hello_agents.memory import MemoryManager, MemoryItem, MemoryConfig
...
def create_system_prompt(name: str, role: Dict[str, str]):
    """创建NPC的系统提示词"""
...
class NPCAgentManager:
    ...
    def chat(self, npc_name, message: str, player_id: str = "player"):
...
"""
            ...
            if memory_manager:
                ...
                log_memory_retrieval(
                    npc_name, len(relevent_memories), relevent_memories
                )
    ...
    def _build_memory_context(self, memories: List[MemoryItem]):
        if not memories:
            return ""
