...
async def lifespan(app: FastAPI):
    ...
    # 初始化 state_manager，但只有 interval > 0 才启动后台任务
    state_manager = get_state_manager(settings.NPC_UPDATE_INTERVAL)
    if settings.NPC_UPDATE_INTERVAL > 0:
        await state_manager.start()
        print(f"✅ NPC 自主行为系统已启用（间隔: {settings.NPC_UPDATE_INTERVAL}s）")
    else:
        print("ℹ️ NPC 自主行为系统已禁用（NPC_UPDATE_INTERVAL <= 0）")
